  ...
  <div>
    <div class="fly-home fly-panel" style="background-image: url();">
      ...
      <h1>
        ...
        <i
          class="layui-badge fly-badge-vip">{{userInfo.isVip === '0' ? '非VIP': 'VIP' + userInfo.isVip}}</i>
        <!--
            ...
            <ul class="jie-row">
              <li v-for="(item,index) in postList" :key="'postlist' + index">
                ...
                <router-link
                  ...
                  :to="{name:'detail', params: {tid: item._id}}">{{item.title}}</router-link>
            ...
            <ul class="home-jieda">
              <li v-for="(item,index) in commentList" :key="'comments' + index">
                <p>
                  ...
                  在
                  <router-link
                    :to="item.tid? {name: 'detail', params: {tid: item.tid? item.tid._id : ''}} : {name: '404'}">{{item.tid ? item.tid.title: '(用户已经删除该贴)'}}</router-link>中回答：
                </p>
